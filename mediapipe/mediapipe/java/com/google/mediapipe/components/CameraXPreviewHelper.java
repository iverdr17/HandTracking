...
public class CameraXPreviewHelper extends CameraHelper {
  ...
  public Size computeDisplaySizeFromViewSize(Size viewSize) {
    Log.d(TAG, "heightxx: "+viewSize.getHeight()+"width"+viewSize.getWidth());
  ...
  private Size getOptimalViewSize(Size targetSize) {
    if (cameraCharacteristics != null) {
      ...
      float selectedAspectRatioDifference = 1e3f;//1000
      float targetAspectRatio = targetSize.getWidth() / (float) targetSize.getHeight(); //1080w*1969h ratio=.545018
      ...
      for (Size size : outputSizes) {
        ...
        if (aspectRatioDifference <= selectedAspectRatioDifference) {
          Log.d(TAG, "getOptimalViewSize: "+ frameSize.getWidth() +" "+  frameSize.getHeight());
...
public float[] focallengthinmm(){
  float focalLengthMm =
          cameraCharacteristics.get(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS)[0];
  float sensorWidthMm =
          cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_PHYSICAL_SIZE).getWidth();
  float sensorHeightMm =
          cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_PHYSICAL_SIZE).getHeight();
  int activearrayW = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE).width();
  int activearrayH = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE).height();
  int pixelearrayW = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_PIXEL_ARRAY_SIZE).getWidth();
  int pixelearrayH = cameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_PIXEL_ARRAY_SIZE).getHeight();
  Log.d(TAG, "activearray: "+activearrayW+ activearrayH+pixelearrayW+pixelearrayH);
  float[] ans = new float[3];
  ans[0] = focalLengthMm;
  ans[1] = sensorWidthMm;
  ans[2] = sensorHeightMm;
  return ans;
